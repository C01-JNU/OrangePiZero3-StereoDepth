...
bool CpuStereoMatcher::createMatcher() {
    try {
        if (algorithm_ == "sgbm") {
            ...
            matcher_ = sgbm;
            LOG_INFO("创建StereoSGBM匹配器: blockSize={}, P1={}, P2={}",
            ...
            bm->setDisp12MaxDiff(-1); // 不进行左右一致性检查
            bm->setMinDisparity(min_disparity_);
            matcher_ = bm;
            LOG_INFO("创建StereoBM匹配器: blockSize={}, uniquenessRatio={}",
...
cv::Mat CpuStereoMatcher::compute(const cv::Mat& left, const cv::Mat& right) {
    ...
    return disp16; // CV_16S 或 CV_16U? SGBM输出是CV_16S，但后续除以16使用，保持一致
}
