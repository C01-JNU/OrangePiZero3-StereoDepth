...
int main(int argc, char** argv) {
    ...
    std::string output_dir = "images/calibrated";
    std::string mode_str = "scale_to_fit";  // 默认使用方案2
    bool save_comparison = true;
    
    if (argc >= 2) calibration_file = argv[1];
    if (argc >= 3) test_image_dir = argv[2];
    if (argc >= 4) output_dir = argv[3];
    if (argc >= 5) mode_str = argv[4];
    if (argc >= 6) save_comparison = std::string(argv[5]) == "true";
    ...
    if (argc == 2 && (std::string(argv[1]) == "-h" || std::string(argv[1]) == "--help")) {
        ...
        std::cout << "参数:\n";
        std::cout << "  calibration_file: 标定YAML文件路径 (默认: calibration_results/stereo_calibration.yml)\n";
        std::cout << "  test_image_dir: 测试图像目录 (默认: images/test)\n";
        std::cout << "  output_dir: 输出目录 (默认: images/calibrated)\n";
