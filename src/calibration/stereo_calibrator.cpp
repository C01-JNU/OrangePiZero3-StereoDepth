...
bool StereoCalibrator::loadConfiguration() {
    ...
    m_calibrationDir = getAbsolutePath(m_calibrationDir);
    ...
    size_t pos = calibFile.find_last_of('/');
    if (pos != std::string::npos)
        m_outputDir = calibFile.substr(0, pos);
    else
        m_outputDir = ".";
    m_outputDir = getAbsolutePath(m_outputDir);
