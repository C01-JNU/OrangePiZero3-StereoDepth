...
struct ShaderParameters {{
    ...
    struct PipelineParams {{
        uint32_t imageWidth; uint32_t imageHeight; uint32_t maxDisparity; uint32_t windowSize;
        float uniquenessRatio; float penaltyP1; float penaltyP2; uint32_t flags;
        uint32_t speckleWindow; uint32_t speckleRange; uint32_t medianSize; uint32_t padding[3];
        PipelineParams() {{
            ...
            medianSize = MEDIAN_SIZE; padding[0]=padding[1]=padding[2]=0;
        }}
        void print() const {{ printf("PipelineParams: %ux%u, maxDisp=%u, win=%u, flags=0x%x\\n",
               imageWidth, imageHeight, maxDisparity, windowSize, flags); }}
    }};
